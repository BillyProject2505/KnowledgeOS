# CWC Canonical Asset Bible v5.0
...
The following authority boundaries apply throughout CWC-CAB v5.0.
...
canonical_asset_bible: CWC-CAB v5.0
...
This section identifies the canonical project/documentary source for the published CWC-CAB document and establishes its repository location for retrieval, traceability, and controlled publication.
...
| Document Identifier | `CWC-CAB-001` |
| Current Version | `5.0` |
| Canonical Path | `02_Projects/10_CozWeCare/CWC-CAB-001_CWC_Asset_Bible_v5.0.md` |
| Status | `Current Canonical` |

## Authority Boundary

The repository is the canonical project/documentary source for the published CWC-CAB document.
...
When current repository access is available, the current repository instance shall be treated as the current documentary source for CWC-CAB.
...
This protocol defines how AI systems shall retrieve, interpret, apply, validate, and safely use canonical CWC-CAB knowledge.

## Canonical Source Priority

When current repository access is available, the current canonical repository instance shall be preferred over stale conversational or working copies for current CWC-CAB content.

## Canonical Project Source

The current canonical project source is:

`BillyProject2505/KnowledgeOS`

Current CAB path:

`02_Projects/10_CozWeCare/CWC-CAB-001_CWC_Asset_Bible_v5.0.md`

## Source Retrieval

AI systems shall retrieve the current canonical source when repository access is available.

If the source cannot be verified, the AI system shall not silently represent an unverified value as canonical.

## Canonical Precedence

Authority shall be resolved according to the established canonical authority boundaries.

Repository location provides source currency; it does not transfer authority between CWC-CAS, CWC-CAR, CWC-CAB, and CWC-CRS.

## No Silent Inference

AI shall not silently invent, substitute, or promote values that are not explicitly supported by the applicable canonical source.

Where a required canonical value is unavailable or ambiguous, the system shall preserve the ambiguity rather than manufacture a canonical decision.

## Conflict Resolution

When sources conflict, the applicable authoritative canonical source shall take precedence according to the established authority boundary.

Historical references shall be preserved when they describe historical state.

## Asset Retrieval

When an asset specification is required, AI shall retrieve the applicable CWC-CAB specification and its authoritative registry and visual-reference relationships where applicable.

## Content Production

Content production shall apply the canonical asset specifications without silently changing locked identity, geometry, ordering, membership, account identity, or other normative values.

## Visual Production

Visual production shall use CWC-CAB for specification and CWC-CRS for authoritative visual representation, without transferring authority between them.
...
3. preservation of unrelated canonical content;
4. pre-write integrity review;
5. controlled write;
...
AI-applied canonical knowledge shall remain traceable to the authoritative CWC-CAB specification and applicable paired canonical artifacts.
...
|---|---|
| Document Identifier | `CWC-CAB-001` |
| Previous Canonical Version | `4.0` |
| Current Canonical Version | `5.0` |
| Transition Type | Major Canonical Architectural & Specification Revision |
| Previous Status | Previous Canonical Baseline |
| Current Status | Current Canonical |
...
- AI Canonical Usage Protocol;
- Canonical Write Safety;
- controlled current-version reference normalization;
- finalized AS-003 canonical integration;
- cross-platform specification readiness and closure;
- version-history and historical-traceability formalization.
...
Independent specification version
→ Preserve its own version
```

### Historical Preservation

CWC-CAB v4.0 remains the previous canonical baseline.

Historical statements describing the v4.0 canonical state shall remain v4.0 and shall not be rewritten retroactively.
...
| **Document Identifier** | `CWC-CAB-AS-001` |
| **CWC-CRS Document Identifier** | `CWC-CRS-AS-001` |
| **Canonical Asset Bible** | CWC-CAB v5.0 |
...
## 5.4 Canonical Platform Pair Specification

Each platform is represented by one permanent Platform Pair consisting of:
...
Canonical display specifications:

| Property | Canonical Value |
|---|---:|
| Platform Icon Display Box | `25 × 25 px` |
| Icon–Account Gap | `6 px` |
| Account Typeface | `Montserrat` |
| Account Weight | `SemiBold` |
| Account Font Size | `13 px` |
| Account Line Height | `14 px` |
| Inter-Pair Spacing | `24 px` |

The icon and account identity shall remain vertically centered within the canonical 60 px footer platform container.

## 5.5 Canonical Platform Group Distribution

The four Platform Pairs constitute one Canonical Platform Group.

The group shall be distributed:

> **Left → Right**

in the canonical platform order:

```text
Instagram → Facebook → TikTok → WhatsApp
```

The Platform Group shall be centered within the Footer Platform Container.

The canonical inter-pair spacing is `24 px`.

The Platform Group shall not be reordered, arbitrarily redistributed, or independently rearranged during ordinary production.

## 5.6 Platform-Specific Specification Registry

The current platform-specific specification registry is:

| Platform | Specification Units |
|---|---|
| Instagram | `INST-01` → `INST-05` |
| Facebook | `FB-01` → `FB-05` |
| TikTok | `TK-01` → `TK-05` |
| WhatsApp | `WA-01` → `WA-05` |

All four platform groups conform to AS-003.

Platform-specific specifications shall inherit the canonical membership, ordering, account identity, pairing, geometry, spacing, typography, and authority boundaries defined by AS-003.
...
## 15.2A Canonical Pair, Typography, and Spacing

Every approved implementation shall preserve:

- `25 × 25 px` platform icon display bounding box;
- `6 px` icon–account gap;
- `24 px` inter-pair spacing;
- `Montserrat SemiBold`;
- `13 px` account font size;
- `14 px` account line height;
- left-to-right platform group distribution;
- centered platform group within the Footer Platform Container.
...
|---|---|---|
| `4.0` | Previous Canonical | Previous canonical CAB baseline |
| `5.0` | Current Canonical | Major architectural and specification revision consolidating repository-source architecture, AI canonical usage, write safety, controlled version references, and finalized AS-003 |

Historical references to v4.0 remain preserved where they describe historical state.

# 29. Final Canonical Statement

## 29.1 Canonical Asset Definition
...
## 29.3 Canonical Surface Behavior
...
## 29.4 Canonical Production Requirement

Canonical Footer Platforms shall be present in all CWC content production as part of the footer.

## 29.5 Canonical Change
