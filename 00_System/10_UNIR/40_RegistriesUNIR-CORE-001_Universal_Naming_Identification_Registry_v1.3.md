...
**Lock Status:** LOCKED  
**Canonical Scope:** Universal Naming & Identification Registry (UNIR) — consolidated six-Core architecture  
...
This artifact is the v1.3 materialization of the consolidated six-Core UNIR architecture.

### Source Basis

The substantive source basis is the uploaded artifact:

`UNIR-CORE-001(4).md`

Legacy source metadata is not carried forward as current metadata for this release.

### Materialization Method

This release is produced by controlled documentary decomposition:

- substantive six-Core content is retained;
- DIUA-specific architectural material is extracted from this Core artifact;
- no substantive six-Core specification is intentionally redesigned;
- the resulting v1.3 metadata is authored independently of legacy source metadata.

### Scope Boundary

This document contains the consolidated six canonical UNIR Core semantic domains:
...
DIUA-specific architecture and related DIUA-specific material are outside the scope of this Core artifact and are designated for subsequent dedicated materialization.

### Publication Boundary

This artifact is not being published to the repository by this materialization pass. Repository publication remains a separate controlled act.
...
The embedded metadata state has been reconciled with the canonicalization decision recorded below. The prior `INTEGRATED — REVIEW PENDING / NOT YET CANONICAL / UNLOCKED` state represented a stale pre-canonicalization metadata state and is not carried as the current state of UNIR-GRP v1.0.

This reconciliation changes metadata state only. It does not revise the substantive GRP specification or its canonicalization decision.
...
## 10. Excluded / Deferred Material

The following DIUA-specific material is intentionally excluded from this Core artifact through controlled documentary extraction:

- DIUA-specific architectural definition;
- DIUA-specific namespace state;
- DIUA-specific DIC binding;
- DIUA-specific Document Identifier Grammar binding;
- DIUA-specific registration-eligibility material;
- DIUA-specific allocation material.

Their exclusion from this artifact does not constitute DIUA retirement, invalidation, or semantic deletion.

The generic namespace semantics owned by UNIR-NSM remain within this Core document.

A dedicated DIUA document is intended to be materialized subsequently. No future DIUA Document ID is assigned by this artifact.

## 11. Traceability & Materialization Closure
...
UNIR-CORE-001(4).md
    ↓
Controlled Documentary Decomposition
    ├── Six-Core substantive content → retained
    └── DIUA-specific material → extracted
    ↓
UNIR-CORE-001 v1.3
```

This v1.3 artifact preserves the consolidated six-Core architecture while establishing a separate documentary boundary for DIUA-specific material.

No repository publication is authorized by this materialization pass.
...
Further canonical lock requires artifact-level validation and explicit release authorization.
