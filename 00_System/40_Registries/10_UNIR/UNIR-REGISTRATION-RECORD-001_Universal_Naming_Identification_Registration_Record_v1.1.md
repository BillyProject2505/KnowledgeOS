...
source_basis: Validated registration readiness, decision, authorization, allocation, registration-event, reassessment, and concrete Document Identifier allocation evidence, including the current UNIR-REGISTRY-001 v1.0 traceability state and subsequent allocation materialization for DIUA-DIC-000002, DIUA-DIC-000003, and DIUA-DIC-000004
...
| 003 | `DIUA-DIC-000003` | `UG-CORE-001 v1.0` — Universal Governance | ALLOCATED — ACTIVE | Target-document identity/allocation declaration; separate allocation act not independently established in repository |
...
The Universal Governance document explicitly records `DIUA-DIC-000003` as its allocated active Document Identifier. This representation preserves that target-document evidence without inventing an allocation act that has not been independently established.
...
3. concrete Document Identifier allocation reconciliation;
4. identity and authority boundary re-audit;
5. UDS conformance audit and controlled remediation;
6. final integrity verification;
7. explicit canonical lock authorization.
...
The canonical lock applies to the substantive content and approved documentary contract of this artifact. Publication remains a separate operational step.

---

## Governing Principle

> **UNIR-REGISTRATION-RECORD-001 provides a canonical representation and traceability layer for validated UNIR registration records and concrete Document Identifier allocations. It does not replace underlying authoritative records, redefine UNIR Core semantics, or become the current registry-state authority.**
