...
scope: Current registered state of the Universal Naming & Identification Registry, including validated active Concrete Document Identifier allocations
...
source_basis: Validated individual registration, authorization, allocation, reassessment, and concrete Document Identifier allocation artifacts, including current target-document allocation evidence and `UNIR-ALLOCATION-ACT-001` / `UNIR-ALLOCATION-ACT-002`
---
...
**Source Basis:** Validated individual registration, authorization, allocation, reassessment, and concrete identifier-allocation artifacts established through R1–R5, together with subsequent validated target-document allocation evidence and allocation acts  
**Canonical Lineage:** UNIR Core architectural boundary → validated registration artifacts → R1–R5 validation → UNIR-REGISTRY-001 v1.0 → concrete Document Identifier allocation materialization → UNIR-REGISTRY-001 v1.1  
**Supersedes:** `UNIR-REGISTRY-001 v1.0`  
**Superseded By:** None  
...
The substantive registry state in this document is materialized from validated individual registration, authorization, allocation, reassessment, and concrete identifier-allocation artifacts established through the Registry Materialization R1–R5 validation process, together with validated subsequent allocation evidence for the current active Concrete Document Identifier set.
...
### 1.6 Current-State Rule

This revision materially extends the current registry state to include all four known active DIUA/DIC concrete Document Identifier allocations represented by validated evidence:

```text
DIUA-DIC-000001 → UNIS-CORE-001 v1.7
DIUA-DIC-000002 → UA-CORE-001 v0.2
DIUA-DIC-000003 → UG-CORE-001 v1.0
DIUA-DIC-000004 → Universal Document System
```

The four allocations are Concrete Document Identifier assignments and are not UNIR Registry Objects.
...
| 003 | `DIUA-DIC-000003` | `UG-CORE-001 v1.0` — Universal Governance | ALLOCATED — ACTIVE | Target-document allocation declaration; separate allocation act not independently established |
| 004 | `DIUA-DIC-000004` | Universal Document System | ALLOCATED — ACTIVE | `UNIR-ALLOCATION-ACT-002` |

**Current Active Concrete Document Identifier Allocation Count: 4**
...
The Universal Architecture document explicitly records `DIUA-DIC-000002` as `ALLOCATED_ACTIVE` and references `UNIR-ALLOCATION-ACT-001`.
...
**Current State:** ALLOCATED — ACTIVE  
**Registry:** UNIR  
**Allocation Act:** NOT ESTABLISHED IN REPOSITORY
...
The Universal Governance document explicitly records `DIUA-DIC-000003` as `ALLOCATED_ACTIVE`. This registry state records that target-document evidence without inventing a missing allocation act.
...
- individual Document Identifiers other than the explicitly validated allocations `DIUA-DIC-000001` through `DIUA-DIC-000004`;
...
This document references their role in the current registry state and preserves the traceability required to reconstruct the registration chain and current Concrete Document Identifier allocation set.
...
Current Active Document Identifier Allocations represented: 4
...
The document has completed the required materialization, artifact-level audit, UDS conformance review, concrete Document Identifier allocation reconciliation, and post-remediation final integrity check.
...
This lock applies to `UNIR-REGISTRY-001 v1.1` as the current materialized registry-state artifact and supersedes v1.0 for current state representation.
It does not alter `UNIR-CORE-001 v1.3`, nor does it alter any underlying registration, allocation, authorization, reassessment, or historical record.
